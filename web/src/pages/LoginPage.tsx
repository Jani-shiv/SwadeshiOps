  ...
  return (
    <div className="relative min-h-screen overflow-hidden noise-overlay">
      ...
      <div className="relative z-10 grid min-h-screen lg:grid-cols-[1.15fr_0.85fr]">
        <section className="hidden flex-col justify-between border-r border-[var(--color-border)] bg-[var(--color-surface)] p-10 lg:flex xl:p-14">
          <div>
            <div className="mb-8 inline-flex items-center gap-3 rounded-xl border border-[var(--color-border)] bg-slate-900/90 px-4 py-3 shadow-[0_10px_30px_rgba(66,16,44,0.12)] backdrop-blur-xl">
              ...
              <div>
                <p className="text-[16px] font-semibold tracking-tight text-slate-50">SwadeshiOps</p>
                <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">DevOps platform</p>
              </div>
            </div>

            <h1 className="max-w-xl text-5xl font-bold tracking-tight leading-[1.05] tracking-tight text-slate-50">
            ...
            <div className="mt-10 grid max-w-xl gap-4">
              {[
                ...
                <div key={item.title} className="flex items-start gap-4 rounded-2xl border border-[var(--color-border)] bg-slate-900/90 p-4 shadow-[0_10px_25px_rgba(66,16,44,0.12)] backdrop-blur-xl">
                  ...
                  <div>
                    <p className="text-[15px] font-semibold tracking-tight text-slate-50">{item.title}</p>
          ...
          <div className="grid max-w-xl grid-cols-3 gap-4">
            {[
              ...
              <div key={stat.label} className="rounded-2xl border border-[var(--color-border)] bg-slate-900/90 p-4 text-center shadow-[0_10px_25px_rgba(66,16,44,0.12)] backdrop-blur-xl">
                <p className="text-2xl font-semibold tracking-tight tracking-tight text-slate-50">{stat.value}</p>
        ...
        <section className="flex items-center justify-center px-6 py-10 sm:px-8 lg:px-10">
          <div className="w-full max-w-[460px]">
            <div className="mb-8 lg:hidden">
              <div className="mb-4 inline-flex items-center gap-3 rounded-xl border border-[var(--color-border)] bg-slate-900/90 px-4 py-3 shadow-[0_10px_30px_rgba(66,16,44,0.12)] backdrop-blur-xl">
                ...
                <div>
                  <p className="text-[15px] font-semibold tracking-tight text-slate-50">SwadeshiOps</p>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">DevOps platform</p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-[var(--shadow-elevated)] sm:p-8">
              <div className="mb-8 flex items-center justify-between">
                <div>
                  <h2 className="text-3xl font-semibold tracking-tight tracking-tight text-slate-50">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {!isLogin && (
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label className="mb-2 block text-[13px] font-medium text-slate-300">Full Name</label>
                      <input
                        type="text"
                        value={fullName}
                        onChange={(e) => setFullName(e.target.value)}
                        className="w-full rounded-xl border border-[var(--color-border)] bg-slate-900/50 px-4 py-3 text-[13px] text-slate-50 outline-none transition focus:border-[var(--color-accent)] focus:ring-4 focus:ring-[rgba(59,130,246,0.15)]"
                        placeholder="Shiv Jani"
                        required
                      />
                    </div>
                    <div>
                      <label className="mb-2 block text-[13px] font-medium text-slate-300">Username</label>
                      <input
                        type="text"
                        value={username}
                ...
                <div>
                  <label className="mb-2 block text-[13px] font-medium text-slate-300">Email</label>
                  <input
                    type="email"
                    value={email}
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => setPassword(e.target.value)}
                        className="w-full rounded-xl border border-[var(--color-border)] bg-slate-900/50 py-3 pl-10 pr-12 text-[13px] text-slate-50 outline-none transition focus:border-[var(--color-accent)] focus:ring-4 focus:ring-[rgba(59,130,246,0.15)]"
                      placeholder="••••••••"
                      required
                      minLength={8}
                    />
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg p-1 text-slate-500 transition hover:bg-slate-800 hover:text-slate-100"
                    >
